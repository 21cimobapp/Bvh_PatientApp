...
import 'welcome_screen_renderer.dart';

class WelcomeScreen extends StatefulWidget {
  final List<Slide> slides;
  final Function onCityChange;

  const WelcomeScreen({Key key, this.slides, @required this.onCityChange})
      : super(key: key);
...
class WelcomeScreenState extends State<WelcomeScreen>
  ...
  Widget build(BuildContext context) {
    ...
    _cardHeight = (size.height * .75).clamp(300.0, size.height);
    _cardWidth = _cardHeight * .6;
  ...
  bool _handleScrollNotifications(Notification notification) {
    ...
    if (notification is ScrollUpdateNotification) {
      ...
      widget.onCityChange(
          widget.slides
              .elementAt(_pageController.page.round() % widget.slides.length),
          _pageController.page.round() % widget.slides.length);
    }
    //Scroll Start
